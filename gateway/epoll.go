...
// epoll是个池子，类似主reactor，但没用epoll用了个协程去listen
type ePool struct {
	eChan  chan *connection //就绪的连接
	tables sync.Map         // fd -> connection的映射
	eSize  int
	done   chan struct{}

	ln *net.TCPListener                 //监听socket
	f  func(c *connection, ep *epoller) //对应的epoller
}
